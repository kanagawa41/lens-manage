ActiveAdmin.register CollectResult do
  ...
  index do
    ...
    actions defaults: false do |model|
      item 'view', admin_collect_result_path(model), class: 'view_link member_link'
      item 'edit', edit_admin_collect_result_path(model), class: 'edit_link member_link'
      item 'delete', admin_collect_result_path(model), class: 'delete_link member_link', method: :delete, :confirm => "All grades, uploads and tracks will be deleted with this content.Are you sure you want to delete this Content?"
    end
  end
end
