namespace :item do
  ...
  task fetch: :environment do
    ...
    item_ids = HtmlWarehouse.select(:item_id).where(html: nil).limit(MAX_SEARCH_NUM).all.map{|record| record[:item_id]}
    ...
    Capybara.register_driver :poltergeist do |app|
      Capybara::Poltergeist::Driver.new(app, {
        headers: {
          ...
          # HTTP_USER_AGENT: 'Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.115 Mobile Safari/537.36',
    ...
    item_ids.each do |item_id|
      ...
      # イメージURLを取得(MAX４枚)
      session.all(:css, 'div.owl-dot-inner img').each_with_index do |img, i|
